...
class UnitPayModel
{
    ...
    function donateForAccount($account, $count)
    {
		...
		$query = "INSERT INTO `dcode` (member, code, money) VALUES (".$this->mysqli->real_escape_string($account).", {$code}, " . $this->mysqli->real_escape_string($count) . ")";
/*         $query = "
            UPDATE
                ".Config::TABLE_ACCOUNT."
            SET
                ".Config::TABLE_ACCOUNT_DONATE." = ".Config::TABLE_ACCOUNT_DONATE." + ".$this->mysqli->real_escape_string($count).",
				".Config::TABLE_ACCOUNT_OTHER." = ".Config::TABLE_ACCOUNT_OTHER." + ".$this->mysqli->real_escape_string($count)."
            WHERE
                ".Config::TABLE_ACCOUNT_NAME." = '".$this->mysqli->real_escape_string($account)."'
        "; */
