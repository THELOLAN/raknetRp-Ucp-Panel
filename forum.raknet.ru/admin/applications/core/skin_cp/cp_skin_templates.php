 ...
 * Last Updated: $Date: 2013-04-12 20:11:40 -0400 (Fri, 12 Apr 2013) $
 * </pre>
 *
...
<div class='acp-box'>
	<h3>{$this->lang->words['vse_basic_settings']}</h3>
	<table class='ipsTable double_pad'>
		<tr>
...
EOF;
	...
	{	
		...
		if ( $data['set_by_skin_gen'] )
		{
			...
			{
				$canDelete = false;
				$desc .= "<a href='{$this->settings['base_url']}{$this->form_code}&amp;do=cancelVisualEditor&amp;set_id={$data['set_id']}' class='mini_button secondary right ipsCancelEditor'>{$this->lang->words['vse_cancel_vse']}</a>";
			}
			else
			{
				$desc .= "<a href='{$this->settings['base_url']}{$this->form_code}&amp;do=launchVisualEditor&amp;set_id={$data['set_id']}' target='_blank' class='mini_button right'>{$this->lang->words['vse_launch_vse']}</a>";
			}
		}
...
$IPBHTML .= <<<EOF
	...
	<script type="text/javascript">
		var cancelEditorDialogue = new Template("<div class='acp-box'><h3>{$this->lang->words['close_vse_title']}</h3><div class='pad'><p>{$this->lang->words['vse_close_desc']}<br /><br /><a href='#{url}' class='button'>{$this->lang->words['vse_close_linktext']}</a></p></div></div>" );
