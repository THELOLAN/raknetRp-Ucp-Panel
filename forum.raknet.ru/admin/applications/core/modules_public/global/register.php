 ...
 * IP.Board v3.4.8
 * Register
 * Last Updated: $Date: 2014-11-05 17:53:33 -0500 (Wed, 05 Nov 2014) $
 * </pre>
 *
 * @author 		$Author $
...
class public_core_global_register extends ipsCommand
{
	...
	protected function _completeRegistration( $form_errors=array() )
	{
		...
		else
		{
			...
			$this->request['EmailAddress']			= !empty($this->request['EmailAddress'])			? $this->request['EmailAddress']			: ( ( substr( $member['email'], 0, 1 ) === '@' ? '' : $member['email'] ) );
			$this->request['EmailAddress_two']		= !empty($this->request['EmailAddress_two'])		? $this->request['EmailAddress_two']		: ( ( substr( $member['email'], 0, 1 ) === '@' ? '' : $member['email'] ) );
			
			// http://community.invisionpower.com/resources/bugs.html/_/ip-board/partial-registrations-from-windows-live-possibly-others-r41100
			if ( substr( $in_email, 0, 1 ) === '@' )
			{
				$in_email = '';
			}
		}
	...
	{
		/* If the cookie is already set to 'yes' then we've already filled this form in and need to show appropriately */
		$hasCoppa = FALSE;
		if ( IPSCookie::get('coppa') AND IPSCookie::get('coppa') == 'yes' )
		{
			$hasCoppa = TRUE;
		}
		
		$birthday = strtotime( $this->request['month'] ."-" . $this->request['day'] . "-" . $this->request['year']);
		if( $hasCoppa === FALSE AND time() - ( $birthday + ( 60 * 60 * 24 * 365 * 13 ) ) > 0 )
		{
