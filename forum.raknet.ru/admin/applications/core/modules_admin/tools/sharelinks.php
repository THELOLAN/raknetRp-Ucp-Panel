 ...
 * IP.Board v3.4.8
 * Bookmarks
 * Last Updated: $LastChangedDate: 2013-01-07 12:09:37 -0500 (Mon, 07 Jan 2013) $
 * </pre>
 *
 * @author 		$Author: mark $
...
class admin_core_tools_sharelinks extends ipsCommand
{
	...
	*/	
	...
	function sharelinksSave( $type='add' )
	{
		...
		if( count( $errors ) )
		{
	 	...
	 	$data = array( 	 			
	 					...
	 					'share_groups'		=> empty( $this->request['share_groups'] ) ? '*' : implode( ',', $this->request['share_groups'] )
	 				);
	...
	public function sharelinksForm( $type='add' )
	{
		...
		$form['share_groups']		= $this->registry->output->generateGroupDropdown( 'share_groups[]', ( $data['share_groups'] == '*' ? NULL : explode( ',', $data['share_groups'] ) ), TRUE );
