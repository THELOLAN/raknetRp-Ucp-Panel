 ...
 * IP.Board v3.4.8
 * Rebuild & Other Tools
 * Last Updated: $LastChangedDate: 2014-10-01 13:50:54 -0400 (Wed, 01 Oct 2014) $
 * </pre>
 *
...
class admin_core_tools_rebuild  extends ipsCommand
{
		...
		try
		{
 			foreach( new DirectoryIterator( $this->settings['upload_dir'] ) as $file )
 			{
				if ( $file->getFilename() == '.' OR $file->getFilename() == '..' )
				{
 				...
 				else if ( strstr( $file->getFilename(), 'post-' ) )
 				{
 					$i++;
	...
	public function rebuildPosts()
	{
		...
		// Enabled?
		//-----------------------------------------
		if ( !defined( 'LEGACY_UPGRADE' ) OR LEGACY_UPGRADE !== TRUE )
		{
			$this->rebuildSplashScreen();
			return;
		}
		...
		while( $r = $this->DB->fetch( $outer ) )
		{
			...
			switch( $type )
			{
				...
				case 'posts':
					...
					$rawpost = $newParser->BBCodeToHtml( $r['post'] );
				break;
	...
	public function rebuildSplashScreen()
	{
		...
		if ( defined( 'LEGACY_UPGRADE' ) AND LEGACY_UPGRADE === TRUE )
		{
			$todo[] = array( 'posts'	, $this->lang->words['remenu_posts'] );
			$todo[] = array( 'pms'		, $this->lang->words['remenu_pms'] );
			$todo[] = array( 'announce'	, $this->lang->words['remenu_announce'] );
			$todo[] = array( 'sigs'		, $this->lang->words['remenu_sigs'] );
			$todo[] = array( 'aboutme'	, $this->lang->words['remenu_aboutme'] );
			$todo[] = array( 'rules'	, $this->lang->words['remenu_rules'] );

			foreach( ipsRegistry::$applications as $app_dir => $app_data )
			{
				//-----------------------------------------
				// Retrieve the post rebuild extensions
				//-----------------------------------------
