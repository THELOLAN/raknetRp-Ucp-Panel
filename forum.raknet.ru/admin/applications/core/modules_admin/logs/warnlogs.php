 ...
 * $LastChangedDate: 2013-05-02 23:43:16 -0400 (Thu, 02 May 2013) $
 * @version		v3.4.8
 * $Revision: 12217 $
 */
...
class admin_core_logs_warnlogs extends ipsCommand 
{
	...
	protected function _view()
	{
		...
		while ( $row = $this->DB->fetch() )
		{
			...
			$row['_susp']		= ( in_array( $row['wl_suspend'], array( -1, -2 ) ) ) ? $this->lang->words['wlog_indef'] : ( $row['wl_suspend'] == 0 ? $this->lang->words['wlog_none'] : $row['wl_suspend'] . ' ' . $row[ $row['wl_suspend_unit'] ? $row['wl_suspend_unit'] : 'd' ] );
