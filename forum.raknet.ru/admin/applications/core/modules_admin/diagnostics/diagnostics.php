...
class admin_core_diagnostics_diagnostics extends ipsCommand
{
	...
	{	
		$checkdirs = array( IPS_PUBLIC_PATH . 'style_images', 
							...
							IPS_CACHE_PATH . 'cache/tmp/HTML',
							IPS_CACHE_PATH . 'cache/tmp/URI',
							IPS_CACHE_PATH . 'cache/tmp/CSS',
		...
		foreach( $checkdirs as $dir_to_check )
		{
			if( !file_exists( $dir_to_check ) )
			{
				...
				/* tmp/ folders are created automatically. We don't need to make sure they exist but if they do they should still be writable */
				if( strpos( $dir_to_check, 'tmp/' ) )
				{
					continue;
				}
