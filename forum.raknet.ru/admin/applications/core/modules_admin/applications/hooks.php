 ...
 * IP.Board v3.4.8
 * Hooks Management
 * Last Updated: $Date: 2014-09-25 13:57:23 -0400 (Thu, 25 Sep 2014) $
 * </pre>
 *
...
class admin_core_applications_hooks extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry ) 
	{
		...
		if( defined('NO_WRITE_HOOKS') )
		{
			$this->registry->output->showError( $this->lang->words['no_write_hooks'] );
		}
...
EOF;
	...
	{
		if ( preg_match( "/class\s+(\S+)\s+extends\s+(\S+)/i", $source, $matches ) )
		{
			$source = str_replace( "extends {$matches[2]}", "extends (~extends~)", $source );
		}
