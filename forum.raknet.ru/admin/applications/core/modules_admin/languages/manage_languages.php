 ...
 * Last Updated: $LastChangedDate: 2013-10-14 17:55:18 -0400 (Mon, 14 Oct 2013) $
 * </pre>
 *
...
class admin_core_languages_manage_languages extends ipsCommand
{
	...
	public function translateKill()
	{
		...
		/* Delete session */
		$this->cache->deleteCache( 'translate_session' );
	...
	{	
		...
		while( $r = $this->DB->fetch() )
		{
			$lang[] = array(
								'id'      => $r['word_id'],
								'default' => nl2br( IPSText::htmlspecialchars( $r['word_default'], ENT_QUOTES ) ),
								'custom'  => IPSText::htmlspecialchars( $r['word_custom'], ENT_QUOTES ),
	...
	public function handleLanguageInformationForm( $mode='new' )
	{
		...
		{
			$errors[] = $this->lang->words['language_title_missing'];
		}
		
		if( ! $this->request['lang_short'] )
		{
			$errors[] = $this->lang->words['language_locale_missing'];
		}
		
		if( count( $errors ) )
		{
			$this->registry->output->global_message = implode( ', ', $errors );
			$this->languageInformationForm( $mode );
			return;
		}
