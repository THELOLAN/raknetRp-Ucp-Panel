...
class admin_core_report_reports extends ipsCommand 
{
	...
	public function _createPlugin()
	{
		...
		if( $_POST['finish'] == 1 )
		{
			...
			if( ! $_POST['plugi_title'] )
			{
				$errors[] = "<li>{$this->lang->words['r_missing_title']}</li>";
			}
			if( ! $_POST['plugi_desc'] )
			{
				$errors[] = "<li>{$this->lang->words['r_missing_desc']}</li>";
			}
			if( ! $_POST['plugi_file'] )
			{
				$errors[] = "<li>{$this->lang->words['r_missing_file']}</li>";
			}
			if( preg_match("/[^a-z0-9\-_]/i", $this->request['plugi_file'] ) )
			{
				$errors[] = "<li>{$this->lang->words['r_incchar']}</li>";
			}
			if( ! is_file( IPSLib::getAppDir( $this->request['appPlugin'] ) . '/extensions/reportPlugins/' . $this->request['plugi_file'] . '.php' ) )
			{
				$errors[] = "<li>{$this->lang->words['r_404file']}</li>";
			}
