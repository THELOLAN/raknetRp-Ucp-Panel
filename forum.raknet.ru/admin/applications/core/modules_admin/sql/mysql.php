...
class admin_core_sql_toolbox_module extends ipsCommand {
	...
	public function sqlRunTool()
	{
		...
		foreach( $tables as $table )
		{
			...
			/* Add to output */
			$output .= $this->html->sqlToolResult( $columns, $rows );
		}
		 ...
		 */
		$this->registry->output->html .= $this->html->sqlToolResultHeader( $columns );
		
		$this->registry->output->html .= $output;
		
		/**
		 * Rikki wanted a stupid header template so we could output his stupid header. This created more work for Brandon.
		 * This in turn meant I had to create a stupid footer to keep other people happy. Must make other people happy. I'm on a horse.
		 */		
		$this->registry->output->html .= $this->html->sqlToolResultFooter();
	...
	public function sqlViewResults( $sql )
	{
		...
		else
		{
			...
			foreach( $the_queries as $key => $sql )
			{
				...
				else if ( preg_match( "/^(?!SELECT)/i", preg_replace( "#\s{1,}#s", "", $sql['sql'] ) ) and ( preg_match( "/admin_login_logs/i", preg_replace( "#\s{1,}#s", "", $sql['sql'] ) ) || preg_match( "/admin_permission_rows/i", preg_replace( "#\s{1,}#s", "", $sql['sql'] ) ) ) )
				{
					$columns[ $queryCntForArray ][]	= $this->lang->words['manual_error'];
				...
				if ( preg_match( "/^SELECT/i", $sql['sql'] ) or preg_match( "/^SHOW/i", $sql['sql'] ) or preg_match( "/^EXPLAIN/i", $sql['sql'] ) )
				{
					...
					while( $r = $this->DB->fetch() )
					{
						...
						for( $i = 0; $i < $cnt; $i++ )
						{
							...
							{
								$row[] = nl2br( IPSText::htmlspecialchars( wordwrap( $r[ $fields[$i]->name ] , 50, "\n", 1 ) ) );
							}
							else
							{
								$row[] = IPSText::htmlspecialchars( $r[ $fields[$i]->name ] );
							}
						}
