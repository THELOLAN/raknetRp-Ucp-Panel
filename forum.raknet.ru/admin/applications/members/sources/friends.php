...
class profileFriendsLib
{
	...
	public function addFriend( $friend_id, $from_id=0, $forceApproval=false, $sendNotification=true )
	{
		...
		{
			/* We may have friend record but it could just be a request awaiting approval. If so just approve it
			 * http://community.invisionpower.com/resources/bugs.html/_/ip-board/bug-with-friends-request-friend-doesnt-appear-in-list-r43134
			 */
			if( !$friend_check['friends_approved'] )
			{
				$this->DB->update( 'profile_friends', array( 'friends_approved' => 1 ), "friends_member_id = {$from_id} AND friends_friend_id={$friend_id}" );
