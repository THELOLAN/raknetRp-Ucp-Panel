 ...
 * IP.Board v3.4.8
 * Member list
 * Last Updated: $Date: 2015-05-08 05:55:54 -0400 (Fri, 08 May 2015) $
 * </pre>
 *
...
class public_members_list_view extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		$this->max_results	= !empty($this->request['max_results']) ? IPSText::parseCleanValue( $this->request['max_results'] ) : $this->max_results;
		$this->sort_key		= !empty($this->request['sort_key']) ? IPSText::parseCleanValue( $this->request['sort_key'] )	: 'members_display_name';
		$this->sort_order	= !empty($this->request['sort_order']) ? IPSText::parseCleanValue( $this->request['sort_order']  ): 'asc';
		...
		$this->request['name_box']	= isset($this->request['name_box']) ? IPSText::parseCleanValue( $this->request['name_box'] ) : '';
		$this->request['quickjump']	= isset($this->request['quickjump']) ? IPSText::parseCleanValue( $this->request['quickjump'] ) : '';
			...
			foreach( $this->custom_fields->out_fields as $id => $data )
			{
				if ( !empty($this->request[ 'field_' . $id ]) )
				{
					...
					if( is_array($this->request[ 'field_' . $id ]) )
					{
						...
						{
							$this->request[ 'field_' . $id ][ $k ] = IPSText::parseCleanValue( urldecode($v) );
					...
					else
					{
						...
						$this->request[ 'field_' . $id ] = IPSText::parseCleanValue( urldecode($this->request[ 'field_' . $id ]) );
					}
