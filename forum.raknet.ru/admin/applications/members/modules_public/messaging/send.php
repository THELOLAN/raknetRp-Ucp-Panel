 ...
 * IP.Board v3.4.8
 * Online list
 * Last Updated: $Date: 2015-04-24 13:16:25 -0400 (Fri, 24 Apr 2015) $
 * </pre>
 *
...
class public_members_messaging_send extends ipsCommand
{
	...
	protected function _sendNewPersonalTopic()
	{
		...
		try
		{
			/* Make sure this was the user who owned the topic if they are editing a draft */
			if( $topicID )
			{
				if( $this->messengerFunctions->canAccessTopic( $this->memberData['member_id'], $topicID ) !== TRUE )
				{
	...
	protected function _showNewTopicForm( $errors='' )
	{
		...
		else
		{
 		...
 		if ( $preview )
 		{ 
 			$displayData['message'] = $_editor->htmlPurify( $_POST['Post-NS'] );
