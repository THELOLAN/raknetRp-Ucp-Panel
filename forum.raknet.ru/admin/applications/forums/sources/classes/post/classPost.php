 ...
 * IP.Board v3.4.8
 * Moderator actions
 * Last Updated: $Date: 2015-02-23 06:21:10 -0500 (Mon, 23 Feb 2015) $
 * </pre>
...
class classPost
{
	 ...
	 * @var		array
	 */
    protected $moderator	= array( 'edit_post' => null, 'edit_topic' => null );
	...
	{
		if ( is_array($this->_internalData['Settings']) && count($this->_internalData['Settings']) && array_key_exists( $key, $this->_internalData['Settings'] ) )
		{
			return true;
		}
	...
	public function globalSetUp()
	{
		...
		$this->request['open_time_date']  = isset($this->request['open_time_date']) ? $this->request['open_time_date'] : NULL;
		$this->request['open_time_time']  = isset($this->request['open_time_time']) ? $this->request['open_time_time'] : NULL;
		$this->request['close_time_date'] = isset($this->request['close_time_date']) ? $this->request['close_time_date'] : NULL;
		$this->request['close_time_time'] = isset($this->request['close_time_time']) ? $this->request['close_time_time'] : NULL;
		
		if ( $this->can_set_open_time AND $this->request['open_time_date'] AND $this->request['open_time_time'] )
		{
			$date					= strtotime( $this->request['open_time_date'] );
			list( $hour , $minute ) = explode( ":", $this->request['open_time_time'] );
		...
		if ( $this->can_set_close_time AND $this->request['close_time_date'] AND $this->request['close_time_time'] )
		{
			$date					= strtotime( $this->request['close_time_date'] );
			list( $hour , $minute ) = explode( ":", $this->request['close_time_time'] );
		...
		if ( $this->getAuthor('member_id') AND $this->settings['post_merge_conc'] )
		{
			...
			if ( ( $topic['last_post'] > $time_check ) AND ( $topic['last_poster_id'] == $this->getAuthor('member_id') ) )
			{
				...
				$last_post = $this->DB->buildAndFetch( array( 'select' => '*',
																			   'from'   => 'posts',
																			   'where'  => 'pid='.intval($last_pid['maxpid']) ) );
				...
				if ( $last_post['author_id'] == $this->getAuthor('member_id') )
				{
					...
					/* Assign to variable instead of passing reference, PHP Strict standards */
					$errors = $parser->getErrors();
					
					if ( is_array( $errors ) && count( $errors ) )
					{
						$this->_postErrors = 'merge_' . array_pop( $errors );
						$this->showReplyForm();
						return;
					}
	...
	public function replySetUp()
	{
		...
		if ( empty($topic['tid']) )
		{
			...
			if ( $topic['tid'] AND $topic['forum_id'] == $this->getForumID() )
			{
				$this->setTopicData( $topic );
			}
			else
...
EOF;
	...
	public function editPost()
	{
		...
		if ( $this->getIsAjax() === TRUE )
		{
			...
			$this->setSettings( array( 'enableSignature' => ( $this->_originalPost['use_sig'] ) ? 1 : 0,
									   ...
									   'post_htmlstatus' => $this->getSettings('post_htmlstatus') ) );
		...
		if ( $this->can_add_poll )
		{
			...
			{
				/* Remove the poll... but only if we aren't on mobile (see ticket 897586) */
				if ( $this->registry->output->getAsMobileSkin() !== TRUE )
				{
		...
		if ( $this->edit_title == 1 )
		{
			...
			if ( $this->_topicTitle != "" )
			{
				if ( $this->_topicTitle != $topic['title'] OR !$topic['title_seo'] )
				{
					...
					if ( (isset( $this->moderator['edit_topic'] ) AND $this->moderator['edit_topic'] == 1) OR ( $this->getAuthor('g_is_supmod') == 1 ) )
					{
	...
	public function editSetUp()
	{
		...
		if ( $this->getIsAjax() === TRUE )
		{
			$this->setSettings( array( 'enableSignature'	=> intval($this->_originalPost['use_sig']),
									   ...
									   'post_htmlstatus'	=> intval( $this->getSettings('post_htmlstatus') ),
							) 		);
		}
		...
		if ( isset( $this->moderator['edit_post'] ) && ! empty( $this->moderator['edit_post'] ) )
		{
			$_canEdit = 1;
		}
		...
		if ( ! $this->getAuthor('g_is_supmod') AND isset( $this->moderator['edit_post'] ) && empty( $this->moderator['edit_post'] ) )
		{
			$perm_id	= $this->getAuthor('org_perm_id') ? $this->getAuthor('org_perm_id') : $this->getAuthor('g_perm_id');
			$perm_array = explode( ",", $perm_id );
	...
	public function sendNewTopicForApprovalEmails( $tid, $title, $author, $post, $pid=0, $type='new' )
	{
		...
		if ( $type == 'new' )
		{
			$template = "new_topic_queue_notify";
		}
		else
		{
			$template = "new_post_queue_notify";
		}
		
		$message = array( 'TOPIC'  => $title,
		...
		foreach( explode( ",", $tmp['notify_modq_emails'] ) as $email )
		{
			if ( $email )
			{
				IPSText::getTextClass('email')->setPlainTextTemplate( IPSText::getTextClass('email')->getTemplate( $template ) );
	...
	public function sendOutTrackedTopicEmails( $topic, $content="" )
	{
		...
		/* Add some quote author info */
		$content = preg_replace( '#<blockquote(.+?)data-author=[\"\'](.+?)[\"\'](.+?)>(.+?)</blockquote>#i', '<blockquote class="ipsBlockquote">\\2:<br />\\4</blockquote>', $content );
		...
		try
		{
			...
			$_like->sendNotifications(
				...
				array(
					...
					'notification_bypass_permission_checks' => $this->_bypassPermChecks,
	...
	public function sendOutQuoteNotifications( $post, $membersToSkip )
	{
		...
		{			
			if ( preg_match_all( '#(\[quote|\<blockquote)(?:[^\]]+?)(?:post|data-cid)=(?:\'|"|&quot;|&\#39;)(\d+?)(?:\'|"|&quot;|&\#39;)#', $post['post'], $quotes) )
			{
				$classToLoad		= IPSLib::loadLibrary( IPS_ROOT_PATH . '/sources/classes/member/notifications.php', 'notifications' );
				$notifyLibrary		= new $classToLoad( $this->registry );
