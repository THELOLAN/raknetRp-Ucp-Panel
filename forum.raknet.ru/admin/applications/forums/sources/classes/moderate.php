 ...
 * IP.Board v3.4.8
 * Moderator actions
 * Last Updated: $Date: 2015-01-26 16:07:28 -0500 (Mon, 26 Jan 2015) $
 * </pre>
 *
...
class moderatorLibrary
{
	...
	public function deleteMemberContent( $memberID, $option, $date=0 )
	{
		...
		if ( $topicFind )
		{
			...
			$this->DB->update( 'topics', array( 'approved' => $this->registry->class_forums->fetchTopicHiddenFlag('pdelete'), 'tdelete_time' => IPS_UNIX_TIME_NOW ), $topicFind );
	...
	public function postToggleSoftDelete( $postIDs, $delete=FALSE, $reason='', $topicIDFix=0 )
	{
		...
		if ( count( $_pids ) )
		{
			foreach( $_pids AS $_post_id )
			{
				try
				{
					$this->registry->topics->unAnswerTopicSingle( $_post_id );
				}
				catch( Exception $e ) {}
			}
	...
	public function postDelete($id)
	{
		...
		foreach( array_keys( $posts ) AS $post_id )
		{
			try
			{
				$this->registry->topics->unAnswerTopicSingle( $post_id );
			}
			catch( Exception $e ) {}
		}
	 ...
	 * @return	array		Topic IDs
	 */
	public function postDeleteFromDb( $id, $nostats=false, $quick=false, $taskInitiated=FALSE )
	{
		$posts			= array();
		$attach_tid		= array();
		$attach_ids		= array();
		...
		foreach( array_keys($posts) as $post )
		{
			...
			try
			{
				$this->registry->topics->unAnswerTopicSingle( $post );
			}
			catch( Exception $e ) {}
		}
	 ...
	 * @return	boolean
	 */
	public function topicDeleteFromDB( $id, $nostats=0, $taskInitiated=FALSE )
	{
		$posts			= array();
		$attach			= array();
		$this->error	= "";
		...
		$class_attach->bulkRemoveAttachment( $posts, 'attach_rel_id', $taskInitiated );
