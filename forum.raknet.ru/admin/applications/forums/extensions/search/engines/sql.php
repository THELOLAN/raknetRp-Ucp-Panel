...
class search_engine_forums extends search_engine
{
	...
	{		
		...
		$sortType		= '';
		$cType			= IPSSearchRegistry::get('contextual.type');
		$cId			= intval( IPSSearchRegistry::get('contextual.id' ) );
		$search_tags	= IPSSearchRegistry::get('in.raw_search_tags');
		...
		if( $search_term )
		{
			...
			if( $content_title_only )
			{			
				$where_clause[] = $this->DB->buildSearchStatement( 't.title', $search_term, true, false, ipsRegistry::$settings['use_fulltext'], ipsRegistry::$settings['s_andor_type'] );
			...
			else
			{
				if ( $onlyPosts )
				{
					$where_clause[] = $this->DB->buildSearchStatement( 'p.' . $this->table['post'], $search_term, true, false, ipsRegistry::$settings['use_fulltext'], ipsRegistry::$settings['s_andor_type'] );
					IPSSearchRegistry::set('set.returnType', 'pids' );
				}
				else
				{
