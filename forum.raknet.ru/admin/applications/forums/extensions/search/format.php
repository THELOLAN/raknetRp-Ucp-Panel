 ...
 * Last Updated: $Date: 2015-03-16 17:25:51 -0400 (Mon, 16 Mar 2015) $
 * </pre>
 *
 * @author 		$author$
...
class search_format_forums extends search_format
{
	...
	public function __construct( ipsRegistry $registry )
	{
		...
		/* Get live or archive */
		if ( ! is_array( ipsRegistry::$request['search_app_filters'] ) )
		{
			ipsRegistry::$request['search_app_filters'] = array();
		}
		...
		if ( count( $ids ) )
		{
			...
			if ( count( $_rows ) )
			{
				...
				foreach( $_rows as $id => $row )
				{
					...
					if ( $this->settings['search_method'] != 'sql' )
					{
						/* Can we read? */
