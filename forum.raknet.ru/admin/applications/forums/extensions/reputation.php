 ...
 * Last Updated: $Date: 2014-10-16 12:59:33 -0400 (Thu, 16 Oct 2014) $
 * </pre>
 *
 * @author 		$author$
...
class reputation_forums
{
			...
			return array( 'type'   => 'pid',
						  ...
						  'inner'  => array( 'select'   => 'p.pid',
						  					...
						  					'add_join' => array( array( 'select' => '',
						  												'from'   => array( 'topics' => 't' ),
						  												'where'  => 't.tid=p.topic_id AND t.approved=' . ipsRegistry::getClass('class_forums')->fetchTopicHiddenFlag( 'visible' ) . ' AND ' . ( empty( $allowedForumIDs ) ? '1=0' : "t.forum_id IN(" . implode( ',', $allowedForumIDs ) . ")" ),
		...
		else
		{
			...
			return array(
				...
				'where'		=>	"r.app='forums' AND r.type='pid' AND " . // belongs to this app
								...
								( empty( $allowedForumIDs ) ? '1=0' : "t.forum_id IN(" . implode( ',', $allowedForumIDs ) . ")" ), // we have permission to view
				);
		}
	}
	...
	public function process( $row )
	{
		...
		if ( ipsRegistry::getClass('permissions')->check( 'download', $forumData ) === FALSE )
		{
			ipsRegistry::$settings['show_img_upload'] =  0;
		}
