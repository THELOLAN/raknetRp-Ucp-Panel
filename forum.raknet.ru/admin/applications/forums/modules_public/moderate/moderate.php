 ...
 * IP.Board v3.4.8
 * Moderator actions
 * Last Updated: $Date: 2014-11-11 15:53:26 -0500 (Tue, 11 Nov 2014) $
 * </pre>
 *
...
class public_forums_moderate_moderate extends ipsCommand
{
	...
	protected function _topicApproveAlter( $type='approve' )
	{
		...
		$this->registry->output->redirectScreen( $this->lang->words['redirect_modified'], $this->settings['base_url'] . $url, $this->request['from'] == 'forum' ? $this->forum['name_seo'] : $this->topic['title_seo'], $this->request['from'] == 'forum' ? 'showforum' : 'showtopic' );
	}
	...
	protected function _multiPostModify()
	{
		...
		else if ( $this->request['pid'] )
		{
			if ( $this->request['nr'] )
			{
				$this->registry->output->silentRedirect( $this->settings['base_url'] . "findpost=" . $this->request['pid'] . '&amp;t=' . $this->request['t'] );
			}
			else
			{
				$this->registry->output->redirectScreen( $this->lang->words['cp_redirect_posts'], $this->settings['base_url'] . "findpost=" . $this->request['pid'] . '&amp;t=' . $this->request['t'] );
			}
		}
		else if ( $this->request['postdate'] )
		{
			if ( $this->request['nr'] )
			{
				$this->registry->output->silentRedirect( $this->settings['base_url'] . "findpost=" . $this->request['postdate'] . '&amp;t=' . $this->request['t'] );
			}
			else
			{
				$this->registry->output->redirectScreen( $this->lang->words['cp_redirect_posts'], $this->settings['base_url'] . "findpost=" . $this->request['postdate'] . '&amp;t=' . $this->request['t'] );
			}
		}
	...
	protected function _multiMovePost( $error='' )
	{
		...
		else
		{
			...
			if( in_array( $move_to_topic['topic_archive_status'], array( $this->registry->class_forums->fetchTopicArchiveFlag( 'working' ), $this->registry->class_forums->fetchTopicArchiveFlag( 'archived' ), $this->registry->class_forums->fetchTopicArchiveFlag( 'restore' ) ) ) )
			{
				$this->request[ 'checked'] =  0 ;
				$this->_multiMovePost( $this->lang->words['cmp_archived_topic'] );
			}
	...
	protected function _deletePost()
	{
		...
		$posts = $this->registry->getClass('topics')->getPosts( array(  'topicId' 		 => $this->topic['tid'],
						                                                'limit'			 => 1,
																		'skipForumCheck' => true,
																		'dateIsLess'     => $post['post_date'],
																		'onlyViewable'   => true,
																		'onlyVisible'    => true,
																		'notIgnored'	 => true,
																		'parse'		  	 => false,
		...
		$redirectPost = array_keys($posts);
		$redirectPage = $this->registry->getClass('topics')->postToPage( $redirectPost[0] );
		
		$this->registry->output->redirectScreen( $this->lang->words['post_deleted'], $this->settings['base_url'] . "showtopic=" . $this->topic['tid'] . "&amp;page=" . intval($redirectPage) . '#entry' . $redirectPost[0], $this->topic['title_seo'], 'showtopic' );
	}
	...
	protected function _getIds( $field='selectedtids', $fieldJS='' )
	{
		...
		{
			if ( is_array( $this->request[ $fieldJS ] ) )
			{
				$jsIds = $this->request[ $fieldJS ];
			}
			else
			{
