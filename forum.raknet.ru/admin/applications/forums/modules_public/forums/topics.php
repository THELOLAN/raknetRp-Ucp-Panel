 ...
 * IP.Board v3.4.8
 * Topic View
 * Last Updated: $Date: 2014-11-18 19:28:54 -0500 (Tue, 18 Nov 2014) $
 * </pre>
 *
...
class public_forums_forums_topics extends ipsCommand
{
	...
	*/
		...
		if ( $this->request['view'] )
		{
			...
			else if ($this->request['view'] == 'new')
			{
				...
				if ( $this->DB->getTotalRows() )
				{
					...
					$this->registry->output->silentRedirect( $this->settings['base_url']."showtopic=".$topicData['tid'], $topicData['title_seo'], 302, 'showtopic' );
				}
				else
				{
					$this->registry->output->showError( 'topics_none_newer', 10356, null, null, 404 );
			...
			else if ($this->request['view'] == 'findpost')
			{
				...
				if ( $this->registry->getClass('topics')->isArchived( $topicData ) && $this->registry->class_forums->fetchArchiveTopicType( $topicData ) != 'working' )
				{
					$cposts['posts'] = $this->archiveReader->getPostCount( $topicData['tid'], $_perms );
				}
				else
				{
					$query	= ' AND ' . $this->registry->class_forums->fetchPostHiddenQuery('visible');
	...
	protected function _getSameTaggedData()
	{
		...
		/* Have we got tags? */
		if ( $this->settings['forums_enabled_also_tagged'] && $this->settings['tags_enabled'] && isset( $topicData['tags']['tags'] ) && count( $topicData['tags']['tags'] ) )
		{
	...
	public function topicSetUp( $topicData )
	{
		...
		/* PHP will sometimes return the value of a cookie as "deleted" rather than empty. Most browsers seem to understand this, but others (Safari) doesn't */
		if ( $this->request['selectedpids'] == 'deleted' )
		{
			$this->request['selectedpids'] = '';
		}
