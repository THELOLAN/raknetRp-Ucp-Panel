...
class public_forums_ajax_topics extends ipsAjaxCommand
{
	...
	protected function _quote()
	{
		...
		foreach( $posts as $post )
		{
			...
			if ( $this->settings['strip_quotes'] )
			{
				...
				if ( ! preg_match( '#\S+?#', trim( IPSText::stripTags( $post['post'], '<img>' ) ) ) ) // Allow image tags, @link http://community.invisionpower.com/resources/bugs.html/_/ip-board/quote-nesting-off-but-r42302
				{
					$post['post']  = $oldPost;
				}
					...
					(
							...
							(
									...
									(
											...
											or
											$this->memberData['forumsModeratorData'][ $forumData['id'] ]['split_merge']
									)
							)
					)
					{
	...
	protected function _topicPreview()
	{
		...
		else if ( $topic['topic_posts'] && ! $isArchived )
		{
			...
			{
				$count = $this->DB->buildAndFetch( array( 'select' => 'COUNT(*) as count, MAX(' . $this->registry->topics->getPostTableField($this->settings['post_order_column'], $isArchived ) . ') as max, MIN('.$this->settings['post_order_column'].') as min',
			...
			{
				$count = $this->DB->buildAndFetch( array( 'select' => 'MAX(' . $this->registry->topics->getPostTableField($this->settings['post_order_column'], $isArchived ) . ') as max',
												  		  'from'   => $this->registry->topics->getPostTableField('_table_', $isArchived ),
			...
			if ( $count['max'] )
			{
				$_posts = $this->registry->topics->getPosts( array( 'onlyViewable'    => true,
														  		    'postId'		  => ( $this->settings['post_order_column'] == 'pid' ) ? array( intval( $count['min'] ), intval( $count['max'] ) ) : array(),
														  		    'postDate'		  => ( $this->settings['post_order_column'] == 'post_date' ) ? array( intval( $count['min'] ), intval( $count['max'] ) ) : array(),
																    'archiveToNative' => true,
														   	        'isArchivedTopic' => $this->registry->topics->isArchived( $topic ) ) );
				
				
				
				foreach( $_posts as $postid => $r )
				{
					$r['tid']		= $topic['tid'];
		...
		{
			/* Set up settings for this post */
			$parser->set( array( 'parseArea'      => 'topics',
						 'memberData'     => $this->memberData,
						 'parseBBCode'    => $this->registry->class_forums->forum_by_id[ $topic['forum_id'] ]['use_ibc'],
						 'parseHtml'      => ( $this->registry->class_forums->forum_by_id[ $topic['forum_id'] ]['use_html'] and $this->caches['group_cache'][ $data['member_group_id'] ]['g_dohtml'] and $data['post_htmlstate'] ) ? 1 : 0,
						 'parseEmoticons' => $data['use_emo'] ) );
	...
	protected function _postApproveToggle()
	{
		...
		if ( ! $_yoGo )
		{
			$this->returnJsonArray( array( 'error' => 'no_permission' ) );
		}
	...
	public function editBoxSave()
	{
		...
		try
		{
			...
			$parser->set( array( 'memberData'     => $this->postClass->getAuthor(),
								 ...
								 'parseArea'	  => 'topics',
					  			 'parseHtml'      => ( $this->registry->getClass('class_forums')->forum_by_id[ $fid ]['use_html'] and $this->memberData['g_dohtml'] and $this->request['post_htmlstatus'] ) ? 1 : 0,
