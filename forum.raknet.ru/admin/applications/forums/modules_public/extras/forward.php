 ...
 * Last Updated: $Date: 2014-09-05 12:52:36 -0400 (Fri, 05 Sep 2014) $
 * </pre>
 *
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		if ( $this->request['url'] AND $this->request['title'] )
		{
			...
			if ( ! empty( $this->settings['ccs_root_url'] ) )
			{
				...
				{
					if ( ! empty($_url) AND parse_url( $this->page['url'], PHP_URL_HOST ) === parse_url( $_url, PHP_URL_HOST ) )
					{
						$ok = TRUE;
						break;
					}
			...
			{
				if ( parse_url( $this->page['url'], PHP_URL_HOST ) !== parse_url( $this->settings['_original_base_url'], PHP_URL_HOST ) ) 
				{
