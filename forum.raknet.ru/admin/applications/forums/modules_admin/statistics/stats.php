 ...
 * IP.Board v3.4.8
 * Forum Statistics
 * Last Updated: $Date: 2013-04-19 19:48:56 -0400 (Fri, 19 Apr 2013) $
 * </pre>
 *
...
class admin_forums_statistics_stats extends ipsCommand
{
	...
	*/
	...
	public function result_screen($mode='reg')
	{
		...
		{
			$php_date	= preg_replace( '#(?<!%)((?:%%)*)%e#', '\1%#d', $php_date );
		}
		...
		foreach( $results as $data )
		{
    		...
    		{
    			$data['_name'] = $this->lang->words['stats_weekno'] . strftime( $php_date, $data['result_maxdate'] );
    		}
    		else
    		{
    			$data['_name'] = strftime( $php_date, $data['result_maxdate'] );
    		}
