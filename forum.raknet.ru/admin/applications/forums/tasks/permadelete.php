 ...
 * $Author: stuarts $
 * @since		-
 * $LastChangedDate: 2014-09-26 09:22:53 -0400 (Fri, 26 Sep 2014) $
 * @version		v3.4.8
 * $Revision: 12514 $
 */
...
class task_item
{
	...
	public function runTask()
	{
		...
		while( $r = $this->DB->fetch($outer) )
		{
			...
			$moderatorLibrary->topicDeleteFromDB( $r['tid'], true, true );
		...
		while( $r = $this->DB->fetch($outer) )
		{
			...
			$moderatorLibrary->init( $this->registry->getClass('class_forums')->allForums[ $r['forum_id'] ], $r );
			$moderatorLibrary->postDeleteFromDb( $r['pid'], true, false, true );
