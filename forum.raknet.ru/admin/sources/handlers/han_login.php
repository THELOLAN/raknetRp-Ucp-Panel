 ...
 * Last Updated: $Date: 2013-09-05 21:09:43 -0400 (Thu, 05 Sep 2013) $
 * </pre>
 *
...
class han_login
{
	...
	public function loginWithoutCheckingCredentials( $memberID, $setCookies=TRUE )
	{
		...
		return array( $this->registry->getClass('class_localization')->words['partial_login'], str_replace( '?&', '?', $url ) );
	}
	...
	public function verifyLogin()
	{
		...
		if ( $this->request['rememberMe'] )
		{
			...
			IPSCookie::set( "pass_hash"   , $member['member_login_key'], $_sticky, $_days, FALSE, TRUE );
			IPSCookie::set( "ipsconnect_" . md5( $this->settings['board_url'] . '/interface/ipsconnect/ipsconnect.php' ), '1', $_sticky, $_days, FALSE, FALSE );
			foreach( $this->caches['login_methods'] AS $k => $method )
			{
				if ( $method['login_folder_name'] == 'ipsconnect' AND $method['login_enabled'] )
				{
					$ipsConnectSettings = unserialize( $method['login_custom_config'] );

					IPSCookie::set( "ipsconnect_" . md5( $ipsConnectSettings['master_url'] ), '1', $_sticky, $_days, FALSE, FALSE );
				}
			}
		}
		else
	 ...
	 */
  	public function loginAuthenticate( $username, $email_address, $password )
  	{
		foreach( $this->modules as $k => $obj_reference )
		{
			...
			$this->revalidate_url 	= ( $obj_reference->revalidate_url ) ? $obj_reference->revalidate_url : $this->revalidate_url;
	...
	public function changeName( $old_name, $new_name, $email_address, $memberData, $type='name' )
  	...
  	public function validateAccount( $member )
  	{
	  	...
	  	foreach( $this->modules as $k => $obj_reference )
		{
			if( method_exists( $obj_reference, 'validateAccount' ) )
			{
				$obj_reference->validateAccount( $member );
				$this->return_code 		= $obj_reference->return_code;
			}
  		}
  	}
  	...
  	public function deleteAccount( $ids )
  	{
	  	...
	  	foreach( $this->modules as $k => $obj_reference )
		{
			if( method_exists( $obj_reference, 'deleteAccount' ) )
			{
				$obj_reference->deleteAccount( $ids );
				$this->return_code 		= $obj_reference->return_code;
			}
  		}
  	}
