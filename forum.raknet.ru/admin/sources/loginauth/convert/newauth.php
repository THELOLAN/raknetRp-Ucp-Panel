...
class converters
{
	...
	private function bbpress($username, $email, $password)
	{
		...
		if ( strlen( $hash ) == 32 )
		{
			$success = (bool) ( md5( $password ) == $this->parent->_memberData['conv_password'] );
		}
	...
	private function bbpress23($username, $email, $password)
	{
		return $this->bbpress( $username, $email, $password );
	}
	...
	private function discuz($username, $email, $password)
	{
	 ...
	 * FusionBB
	 **/
	private function fusionbb( $username, $email, $password )
	{
		/* FusionBB Has multiple methods that can be used to check a hash, so we need to cycle through them */

		/* md5( md5( salt ) . md5( pass ) ) */
		if ( md5( md5( $this->parent->_memberData['misc'] ) . md5( $password ) ) === $this->parent->_memberData['conv_password'] )
		{
			return TRUE;
		}

		/* md5( md5( salt ) . pass ) */
		if ( md5( md5( $this->parent->_memberData['misc'] ) . $password ) === $this->parent->_memberData['conv_password'] )
		{
			return TRUE;
		}

		/* md5( pass ) */
		if ( md5( $password ) === $this->parent->_memberData['conv_password'] )
		{
			return TRUE;
		}
	...
	private function phpbb($username, $email, $password)
	{
		...
		// Are we successful? If not - make sure the board hasn't been upgraded from phpBB2 and the user
		// just hasn't logged in yet to get their pass converted to phpBB3 format.

		if ( ! $success )
		{
			$success = ( $this->parent->hashCryptPrivate( $single_md5_pass, $hash, $itoa64 ) === $hash ? true : false );
		}
	 ...
	 * Vanilla
	 **/
	private function vanilla($username, $email, $password)
	{
		$success = FALSE;
		...
		{
			$success = ($this->parent->_memberData['conv_password'] == md5(md5(str_replace('&#39;', "'", html_entity_decode($password))) . $this->parent->_memberData['misc'])) ? TRUE : FALSE;
		}
		// New pass hash check
		else
		{
	 ...
	 * vBulletin 5.1
	 **/
	private function vb51connect($username, $email, $password)
	{
		/* Which do we need to use? */
		$algo = explode( ':', $this->parent->_memberData['misc'] );
		...
		switch( $algo[0] )
		{
			case 'blowfish':
				...
				return $ph->CheckPassword( $md5_once_password, $this->parent->_memberData['conv_password'] );
			break;

			case 'legacy':
				/* Legacy Passwords are stored in a format of HASH SALT so we need to explode on the space. */
				$token = explode( ' ', $this->parent->_memberData['conv_password'] );
				return $this->vbulletin( $username, $email, $password, $token[1], $token[0] );
			break;
		}
	}
	 ...
	 * vBulletin 5
	 **/
	private function vb5connect($username, $email, $password)
	{
		if ( strpos( $this->parent->_memberData['misc'], 'blowfish' ) === FALSE AND strpos( $this->parent->_memberData['misc'], 'legacy' ) === FALSE )
		{
			return $this->vbulletin( $username, $email, $password );
		}
		else
		{
			return $this->vb51connect( $username, $email, $password );
		}
	}
	 ...
	 * vBulletin 4
	 *
	 * @note Amended 6/11/2014 to allow for vB 5.1 updated passwords, but still have users who have the legacy algo
	 **/
	private function vbulletin($username, $email, $password, $salt=NULL, $hash=NULL)
	{
		if ( is_null( $salt ) )
		{
			$salt = $this->parent->_memberData['misc'];
		}
		...
		$password = html_entity_decode( $password );
		if ($hash == md5(md5(str_replace('&#39;', "'", $password)) . $salt))
		{
			return true;
		}
		else
	...
	/**
	 * Telligent Community Server authentication
	 * Wrap existing Community Server authentication
	 *
	 * @access	private
	 * @param	string		Username
	 * @param	string		Password
	 * @return	boolean		Successful
	 */
	private function telligent_cs( $username, $password )
	{
		return $this->cs($username, $password);
	}
	...
	private function xenforo( $username, $email, $password )
	{
		...
		// XF 1.2
		if ( $this->xenforo12( $username, $email, $password ) )
		{
			return true;
		}

		// XF 1.1
		if ( $this->xenforo11( $username, $email, $password ) )
		{
			return true;
		}
	 ...
	 * @return	boolean		Successful
	 */
	private function xenforo12( $username, $email, $password )
	{
		if ( !isset( $this->settings['xenforo_password_iterations'] ) )
		{
			$this->settings['xenforo_password_iterations'] = 10;
		}

		$ph = new PasswordHash( $this->settings['xenforo_password_iterations'], false );

		if ( $ph->CheckPassword( $password, $this->parent->_memberData['conv_password'] ) )
		{
			return true;
		}
	 ...
	 * @return	boolean		Successful
	 */
	private function xenforo11( $username, $email, $password )
	{
...
class webWizAuth {
	...
	static private function HexToBinary($btHex)
	{
		switch ($btHex)
		{
			case "0":
	...
	static private function BinaryToHex($strBinary)
	{
		switch ($strBinary)
		{
			case "0000":
	...
	static private function WordToBinary($strWord)
	{
		$strBinary = '';
	...
	static private function IntToBinary($intNum)
	{
		$intNew=$intNum;
		$strBinary='';
		while($intNew>1)
	...
	static private function PadBinary($strBinary)
	{
	...
	static private function BlockToHex($strBinary)
	{
		$strHex = '';
	...
	static private function DigestHex($strHex,$strH0,$strH1,$strH2,$strH3,$strH4)
	{
	...
	static private function HexAdd($strHex1,$strHex2)
	{
		$n1 = hexdec($strHex1);
		$n2 = hexdec($strHex2);
		$sum = $n1 + $n2;
	...
	static private function BinaryShift($strBinary,$intPos)
	{
	...
	static private function BinaryOR($strBin1,$strBin2)
	{
		$strBinaryFinal = '';
		for ($intPos=1; $intPos<=strlen($strBin1); $intPos=$intPos+1)
		{
	...
	static private function BinaryAND($strBin1,$strBin2)
	{
		$strBinaryFinal = '';
		for ($intPos=1; $intPos<=strlen($strBin1); $intPos=$intPos+1)
		{
	...
	static private function BinaryNOT($strBinary)
	{
	...
	static private function HexBlockToBinary($strHex)
	{
		$strTemp = '';
