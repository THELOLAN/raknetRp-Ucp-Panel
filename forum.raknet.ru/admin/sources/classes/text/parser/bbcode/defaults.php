...
class bbcode_plugin_sharedmedia extends bbcode_parent_main_class
{
	...
	protected function _checkPostingPermissions( $matches )
	{
		...
		if( !$txt OR !$app OR ! IPSLib::appIsInstalled( $app ) OR !$plugin )
		{
			return '';
		}
	...
	protected function _replaceText( $txt )
	{
		...
		foreach( $_tags as $_tag )
		{
			...
			while( ( $this->cur_pos = stripos( $txt, $open_tag, $this->cur_pos ) ) !== false )
			{
				...
				/* Re-encode exclamation points: @link http://community.invisionpower.com/resources/bugs.html/_/ip-board/username-is-now-showing-with-member-tag-if-contains-r43032 */
				$_option = str_replace( "!", '&#33;', $_option );
