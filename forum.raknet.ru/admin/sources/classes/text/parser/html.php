...
class class_text_parser_html extends classes_text_parser
{
	...
	protected function _parseListTag( $tag, $between_text, $opening_tag, $parse_tag )
	{
		...
		if ( ! $list_type )
		{
			/* If we cannot find the list type from the style attribute, try classes */
			$classes = explode( ' ', trim( $this->_getValueOfOption( 'class', $opening_tag ) ) );
			$list_type = array_pop( $classes );
		}
