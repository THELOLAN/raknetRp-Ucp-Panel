...
class classes_archive_restore
{
	...
	public function restore( $pids=array() )
	{
		...
		$posts = $topicClass->getPosts( array( 'postId'          => $pids,
											   'sortField'		 => 'pid',
											   'sortOrder'		 => 'asc',
											   'postType'    	 => array('visible', 'sdelete', 'hidden', 'pdelete', 'oktoremove'),
											   'skipForumCheck'  => true,
		...
		{
			$topics[$k]['_realMaxPid'] = $this->engine->getMaxPidInTid( $k );
		}
		...
		foreach( $topics as $tid => $data )
		{
			 ...
			 */
			if ( $maxes[ $tid ] == $data['_realMaxPid'] )
			{
				$completedTopics[] = $data['tid'];
			}
			else if ( $maxes[ $tid ] > $data['_realMaxPid'] )
			{
