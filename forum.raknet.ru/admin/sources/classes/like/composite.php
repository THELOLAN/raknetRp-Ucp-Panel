 ...
 * $LastChangedDate: 2014-10-12 16:26:57 -0400 (Sun, 12 Oct 2014) $
 * @version		v3.4.8
 * $Revision: 12538 $
 * 
 * <b>Example Usage:</b>
...
abstract class classes_like_composite
{
	...
	public function sendNotifications( $relId, $type, $notificationOpts=array(), &$sentToIds=array() )
	{
		...
		if ( isset( $notificationOpts['notification_bypass_permission_checks'] ) && $notificationOpts['notification_bypass_permission_checks'] === true )
		{
		...
		/* Fetch data by relID */
		$data = $this->getDataByRelationshipId( $relId, false, (array) $notificationOpts['ignore_data'], $type, ipsRegistry::$settings['like_notifications_limit'] );
		...
		foreach( $data as $row )
		{
			...
			/* Make sure we save meta data - #897885 */
			$notifyLibrary->setMetaData( array( 'meta_area' => $row['like_area'], 'meta_id' => $row['like_rel_id'], 'meta_app' => $row['like_app'] ) );
	 ...
	 * @return	@e mixed	Array of likes data OR null
	 */
	public function getDataByRelationshipId( $relId, $skipAnon=true, $ignoreData=array(), $followType=array(), $limit = 500 )
	{
		/* Init */
		$mids	    = array();
		$members    = array();
