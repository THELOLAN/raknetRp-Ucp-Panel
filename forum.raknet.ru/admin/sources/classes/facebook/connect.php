 ...
 * Last Updated: $Date: 2014-11-11 16:19:43 -0500 (Tue, 11 Nov 2014) $
 * </pre>
 *
	...
	*/
	...
	public function resetApi( $token='', $userId='' )
	{
		...
		if ( $this->_userToken AND $this->_userId  )
		{
			try
			{
				$_userData = $this->_api->api( '/v2.1/' . $this->_userId, array( 'access_token' => $this->_userToken ) );
			}
			catch( Exception $e )
			{
				/* Try re-authorising */
	...
	public function revokeAuthorization()
	{
		if ( $this->_userToken AND $this->_userId )
		{
			try
			{
				$val = $this->_api->api( "/v2.1/{$this->_userId}/permissions", 'DELETE', array( 'access_token' => $this->_userToken ) );
			}
			catch( Exception $e )
			{
	...
	public function fetchHasAppPermission( $permission )
	{
		if ( $this->_userToken AND $this->_userId )
		{
			try
			{
				$val = $this->_api->api( "/v2.1/{$this->_userId}/permissions", array( 'access_token' => $this->_userToken, 'permission' => $permission ) );
			}
			catch( Exception $e )
			{
				$this->registry->output->logErrorMessage( $e->getMessage(), 'FB-EXCEPTION' );
	 ...
	 * @return	array
	 */
	public function fetchUserData( $token='', $picType='square' )
	{
		...
		if ( $token AND is_array( $this->_userData ) AND $this->_userData['id'] AND ! isset( $this->_userData['pic'] ) )
		{
			...
			try
			{
	        	$updates	= $this->_api->api( "/v2.1/{$this->_userData['id']}", array( 'access_token' => $token ) );
	        	$photo		= $this->_api->api( "/v2.1/{$this->_userData['id']}/picture", array( 'type' => $picType, 'redirect' => 'false' ) );
	        	$updates['photo'] = $photo['data']['url'];
        	}
        	catch( Exception $e )
        	{ 
        		/* Try re-authorising */
	...
	public function postLinkToWall( $url, $comment='', $shorten=true )
	{
		...
		try
		{
			$this->_api->api( '/v2.1/me/links', 'POST', array( 'access_token' => $this->_userToken, 'link' => $url, 'message' => $comment ) );
		}
		catch( Exception $e )
		{
			$this->registry->output->logErrorMessage( $e->getMessage(), 'FB-EXCEPTION' );
	...
	public function updateStatusWithUrl( $content, $url, $alwaysAdd=false )
	{
		...
		try
		{
			$return = $this->_api->api( "/v2.1/{$this->_userId}/feed", 'POST', array( 'access_token' => $this->_userToken, 'message' => $content ) );
		}
		catch( Exception $e )
		{
			$this->registry->output->logErrorMessage( $e->getMessage(), 'FB-EXCEPTION' );
	...
	public function finishLogin()
	{
		...
		if ( $_REQUEST['code'] )
		{
			...
			if ( is_string( $rToken ) )
			{
				try
				{
					$_userData = $this->_api->api('/v2.1/me', array( 'access_token' => $rToken ) );
				}
				catch( Exception $e )
				{
					/* Try re-authorising */
	...
	public function fetchUserTimeline( $userId=0, $minId=0, $stripReplies=true )
	{
		...
		if ( $this->_userToken AND $userId )
		{
			try
			{
		        $updates = $this->_api->api( "/v2.1/{$userId}/statuses", array( 'access_token' => $this->_userToken ) );
	        }
	        catch( Exception $e )
	        {
	        	$this->registry->output->logErrorMessage( $e->getMessage(), 'FB-EXCEPTION' );
			...
			if ( is_array( $updates['data'] ) AND count( $updates['data'] ) )
			{
