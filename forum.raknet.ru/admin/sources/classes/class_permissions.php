 ...
 * IP.Board v3.4.8
 * ACP Restrictions
 * Last Updated: $Date: 2013-01-22 16:10:36 -0500 (Tue, 22 Jan 2013) $
 * </pre>
 *
...
class class_permissions
{
	...
	public function __construct( ipsRegistry $registry, $permissions=array() )
	{
		...
		else
		{
			...
			//-----------------------------------------
			// The primary group may not be restricted
			// but it may not be able to access ACP either
			//-----------------------------------------
			
			$cache = ipsRegistry::cache()->getCache('group_cache');
			
			if( ! $cache[$this->memberData['member_group_id']]['g_access_cp'] )
			{
				$primaryGroupIsRestricted = TRUE;
			}
