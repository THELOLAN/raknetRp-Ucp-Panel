    ...
    */	
    ...
    public function getDate($date, $method, $norelative=0, $full_relative=0, $overrideOffset=NULL)
    {
		...
		else if ( $this->settings['time_use_relative'] and ( $norelative != 1 ) )
		{
			...
			{
				return str_replace( '{--}', $this->words['time_today'], $this->_fix( @gmstrftime($this->settings['time_use_relative_format'], ($date + $this->offset) ) ) );
			}
			else if  ( $this_time == $this->yesterday_time )
			{
				return str_replace( '{--}', $this->words['time_yesterday'], $this->_fix( @gmstrftime($this->settings['time_use_relative_format'], ($date + $this->offset) ) ) );
			}
			else
			{
				return $this->_fix( @gmstrftime( $format, ($date + $this->offset) ) );
