 ...
 * <pre>
 * Photo routines
 * Last Updated: $Date: 2015-05-01 09:00:27 -0400 (Fri, 01 May 2015) $
 * </pre>
 *
 * @author		$author$
...
class classes_member_photo
{
	...
	public function save( $member, $photoType, $gravatar='', $url='' )
	{
		...
		switch( $photoType )
		{
			...
			case 'gravatar':
				...
				if( IPSText::checkEmailAddress( $gravatar ) !== FALSE )
				{
					$tcbwOptions['tc_s_pic'] = 0;
					$bwOptions['fbc_s_pic']	 = 0;
	...
	public function setTwitterPicture( $member )
	{
		...
		if ( IPSLib::twitter_enabled() && $member['twitter_token'] && $member['twitter_secret'] )
		{
			...
			if ( $userData['profile_image_url'] )
			{
				...
				$content	= $cfm->getFileContents( $large );
	...
	public function setFacebookPicture( $member )
	{
		...
		if (  IPSLib::fbc_enabled() && $member['fb_uid'] )
		{
			...
			$userData = $facebook->fetchUserData( '', 'large' );
			
			if ( $userData['photo'] )
			{
				$large = $userData['photo'];
				$ext   = IPSText::getFileExtension( $userData['photo'] );
				...
				if ( $mokay === false )
				{
					return array( 'final_location'    => $this->registry->output->isHTTPS ? str_replace( "http://", "https://", $userData['photo'] ) : $userData['photo'],
								  'final_width'       => 50,
								  'final_height'      => 50,
								  't_final_location'  => $this->registry->output->isHTTPS ? str_replace( "http://", "https://", $userData['photo'] ) : $userData['photo'],
								  't_final_width'     => 50,
								  't_final_height'    => 50 );
				}
			}
