 ...
 * Last Updated: $LastChangedDate: 2015-05-11 04:44:42 -0400 (Mon, 11 May 2015) $
 * </pre>
 *
...
class adminOutput extends output
{
	 ...
	 * @return	@e void
	 */
	public function enableAcpHTTPS()
	{
		...
		$this->settings['js_app_url']		= str_replace( 'http://', 'https://', $this->settings['js_app_url'] );
	}
	...
	public function getOtherTabKeys()
	{
		...
		if ( ! count( $this->otherTabKeys ) )
		{
			...
			else
			{
				...
				foreach( array( 'lookfeel', 'support', 'reports' ) AS $v )
				{
					if ( ! in_array( $v, $this->tabKeys ) )
					{
						$this->otherTabKeys[] = $v;
					}
				}
	 ...
	 * @return	@e void
	 */
	public function multipleRedirectFinish($text='')
	{
		$this->lang->loadLanguageFile( array( 'admin_tools' ), 'core' );
		
		$text = ! empty( $text ) ? $text :  $this->lang->words['mr_completed'];
					...
					foreach( $module_data['items'] as $id => $item )
					{
						...
						if ( $app_dir == ipsRegistry::$current_application AND ipsRegistry::$request['section'] AND ( ipsRegistry::$request['section'] == $item['section'] ) AND ( ipsRegistry::$current_module == $item['module'] ) )
						{
							...
							if ( ! $_nav_main_done )
							{
								if( !$this->ignoreCoreNav )
								{
									$this->core_nav[] = array( $this->settings['base_url'] . 'module=' . $_current_module . '&amp;section=' . $item['section'], $item['title'] );
								}
