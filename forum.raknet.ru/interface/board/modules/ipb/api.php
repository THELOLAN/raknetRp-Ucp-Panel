...
class API_Server
{
	 ...
	 * @return	string	xml
	 */	
	public function postTopic( $api_key, $api_module, $member_field, $member_key, $forum_id, $topic_title, $post_content, $post_htmlstate )
	{
		//-----------------------------------------
		// INIT
		//-----------------------------------------
		...
		$post_htmlstate			= ( intval( $post_htmlstate ) > 0 ) ? 1 : 0;
		...
		if ( $this->__authenticate( $api_key, $api_module, 'postTopic' ) !== FALSE )
		{
			...
			if ( $post_htmlstate == 1 )
			{
				$_postClass->setPostContentPreFormatted( $UNCLEANED_post_content );
			}
			else
			{
			...
			$_postClass->setSettings( array( 'enableSignature' => 1,
												   'enableEmoticons' => 1,
												   'post_htmlstatus' => $post_htmlstate,
												   'enableTracker'   => 0 ) );
	 ...
	 * @return	string	xml
	 */	
	public function postReply( $api_key, $api_module, $member_field, $member_key, $topic_id, $post_content, $post_htmlstate )
	{
		//-----------------------------------------
		// INIT
		//-----------------------------------------
