 ...
 * Last Updated: $Date: 2015-02-13 15:16:16 -0500 (Fri, 13 Feb 2015) $
 * </pre>
 *
...
class classEmail implements interfaceEmail
{
    ...
    protected function _smtpConnect()
	{
		...
		if ( substr( $this->smtp_host, 0, 6 ) == 'tls://' )
		{
			$tls = TRUE;
			$this->smtp_host = str_replace( 'tls://', '', $this->smtp_host );
		}
		...
		if ( $this->smtp_code == 220 )
		{
			...
			if ( $tls == TRUE )
			{
				$this->_smtpSendCmd( "STARTTLS" );
				
				if ( $this->smtp_code != 220 )
				{
					$this->_smtpError( "STARTTLS Failed" );
					return false;
				}
				
				if ( ! @stream_socket_enable_crypto( $this->smtp_fp, TRUE, STREAM_CRYPTO_METHOD_TLS_CLIENT ) )
				{
					$this->_smtpError( "Failed to enable crypto (TLS)" );
					return false;
				}
			}
