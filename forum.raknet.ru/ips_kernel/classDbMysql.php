...
abstract class db_main_mysql extends dbMain
{
	 ...
	 * @param	string		And or Or search? (for non-fulltext only)
	 * @return	@e string
	 */
	public function buildSearchStatement( $column, $keyword, $booleanMode=true, $returnRanking=false, $useFulltext=true, $addor = 'and' )
	{
		if( !$useFulltext OR strtolower($this->connect_vars['mysql_tbl_type']) != 'myisam' )
		{
			if ( stripos( $keyword, ' ' ) !== FALSE AND $addor == 'or' )
			{
				$where = array();
