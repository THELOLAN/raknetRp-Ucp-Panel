...
if ( user::logged() ) {
	...
	$user = $db->super_query("SELECT `Admin` FROM `members` WHERE `member_id` = '{$uid}' LIMIT 1");
	...
	switch ( $user["Admin"] ) {
		...
		case 6: {
			...
			if ( $param ) {
				...
				if ( $param == "" ) $style->content( "{content}" , "<div class=\"alert alert-danger\">Ошибка: параметр не может быть пустым.</div>" );
					...
					$style->content( "{content}" , "<div class=\"row\">
														...
														<div class=\"col-xs-12 col-sm-12 col-md-5 col-lg-5\">
															<img src=\"base.php?name={$encode}\" class=\"thumbnail\" width=\"95%\">
														</div>
				...
				$style->content( "{content}" , user::databaseadmin( $params ) );
