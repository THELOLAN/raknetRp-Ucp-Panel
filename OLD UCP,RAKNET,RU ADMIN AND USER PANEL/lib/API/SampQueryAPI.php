...
class SampQueryAPI {
	...
	private function toInteger($sData) {
		if($sData === "") return null;
 		...
 		if(isset($sData[1])) $iInteger += (ord($sData[1]) << 8);
 		
 		if(isset($sData[2])) $iInteger += (ord($sData[2]) << 16);
 		
 		if(isset($sData[3])) $iInteger += (ord($sData[3]) << 24);
 		
 		if($iInteger >= 4294967294) $iInteger -= 4294967296;
