...
elseif ( $number == $query['Pnumber'] ) echo "<div class=\"alert alert-danger\">Данный номер уже кем то используется!</div>";
	...
	$db->query("INSERT INTO `rshop` (`userid`, `name`, `time`, `item`) VALUES ('{$uid}', '{$sname}', '" . time() . "', '7')");
	echo "<div class=\"alert alert-success\">Поздравляем с приобритением номера: {$number}!<br />Его стоимость составляет: {$price} руб.</div>";
