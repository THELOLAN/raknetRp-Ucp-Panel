...
if ( $admins == 1 ) 
{
	...
	$month = isset ( $_POST['m'] ) ? $_POST['m'] : "";
	$year = isset ( $_POST['y'] ) ? $_POST['y'] : "";
	$money = isset ( $_POST['money'] ) ? $_POST['money'] : "";
	$money = (int)$money;
	$param = "";
	$trans = $db->query("SELECT * FROM `cash_log` WHERE `m_o_date` = '{$day}.{$month}.{$year}' AND `money` > '{$money}' ORDER BY `m_date` DESC");
	while ( $tr = $db->get_row( $trans ) ) 
	{
		$name = $db->super_query("SELECT `member_id`, `name`, `Level` FROM `members` WHERE `member_id` = '{$tr['toid']}' LIMIT 1");
		$toname = $db->query("SELECT `member_id`, `name`, `Level` FROM `members` WHERE `member_id` = '{$tr['userid']}' LIMIT 1");
		$tonames = $db->super_query("SELECT `member_id`, `name`, `Level` FROM `members` WHERE `member_id` = '{$tr['userid']}' LIMIT 1");
